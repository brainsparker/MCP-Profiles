...
import { YouComClient, type SearchClient } from "../youcom.js";
...
async function runArm(arm: Arm, tasks: GoldTask[], k: number): Promise<ArmResult> {
  ...
  const client: SearchClient = config.apiKey
  ...
  if (!config.apiKey) {
    process.stderr.write(
      "no YDC_API_KEY — running on the You.com hosted free tier; rate limits (~100 queries/day) may slow or truncate the run\n",
    );
  }
